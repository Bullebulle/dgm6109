...
function buildScales(data) {
    ...
    xScale = d3.scaleLinear()
        .domain([0, 10])
        .range([0, svgWidth - (margin.left + margin.right)]);/*Map to inner chart width*/
...
function organizeData(data) { 
    data.sort(function(a, b) {
        return b.emotionScore - a.emotionScore; /*Sort by emotionScore DESC so larger circles render first (prevents overlap)*/
    });
    return data;
}
...
function drawVisualization(data, drawing) {
 ...
 /* Draw circles - Turn each data point into a circle. */
    ...
    /* Create and position main title text */
